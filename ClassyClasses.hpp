...
template <typename T>
requires Concept<T>
class CTest final : virtual public Base // first virtual, then access specifier keyword (if used)
{
    friend class FC;
    friend ::test::FCC;
...
public:
    ...
    // except for nested structures, they should have only 1 space between their access specifiers.
public:
